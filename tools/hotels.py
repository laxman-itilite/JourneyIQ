...
def register_hotel_tools(mcp: FastMCP) -> None:
    ...
    async def cancel_hotel_booking(booking_id: str, reason: str = "") -> str:
        ...
        if not data:
            return (
                f"Unable to process cancellation for booking '{booking_id}'."
            )
    ...
    async def modify_hotel_booking(
        ...
        if not data:
            return (
                f"Unable to process modification for booking '{booking_id}'."
            )
